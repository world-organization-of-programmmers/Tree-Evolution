...
class Tree:
    def __init__(self, start_x, start_y, genom):
        ...
        if isinstance(genom, np.ndarray):
            ...
            self.genom = np.array([[random.randint(0, (self.chromosoms - 1) * 2) for _ in range(4)] for _ in range(self.chromosoms)])
    ...
    def grow(self, all_trees):
        ...
        for outgrowth in outgrowthes:
            ...
            if outgrowth.energy >= 18:
                ...
                if new_outgrowthes[0] < self.chromosoms and is_free((x, y - 1), all_trees) and is_unique((x, y - 1), appended_outgrowthes):
                    appended_outgrowthes.append(Outgrowth(self.outgrowth_color, (x, y - 1), str(new_outgrowthes[0])))
                if new_outgrowthes[1] < self.chromosoms and is_free((x, y + 1), all_trees) and is_unique((x, y + 1), appended_outgrowthes):
                ...
                if new_outgrowthes[2] < self.chromosoms:
                    ...
                    if x == 0 :
                        if is_free((x_end, y), all_trees) and is_unique((x_end, y), appended_outgrowthes):
                            appended_outgrowthes.append(Outgrowth(self.outgrowth_color, (x_end, y), str(new_outgrowthes[2])))
                    else:
                        if is_free((x - 1, y), all_trees) and is_unique((x - 1, y), appended_outgrowthes):
                            appended_outgrowthes.append(Outgrowth(self.outgrowth_color, (x - 1, y), str(new_outgrowthes[2])))
                if new_outgrowthes[3] < self.chromosoms:
                    x_end = setting.width // setting.pixel_size - 1
                    if x == x_end:
                        if is_free((0, y), all_trees) and is_unique((0, y), appended_outgrowthes):
                            appended_outgrowthes.append(Outgrowth(self.outgrowth_color, (0, y), str(new_outgrowthes[3])))
                    else:
                        if is_free((x + 1, y), all_trees) and is_unique((x + 1, y), appended_outgrowthes):
                            appended_outgrowthes.append(Outgrowth(self.outgrowth_color, (x + 1, y), str(new_outgrowthes[3])))
